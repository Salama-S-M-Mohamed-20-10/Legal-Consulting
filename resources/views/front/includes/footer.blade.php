<!--start footer-->
<footer>
          <div class="fot text-center">
            <div class="container">
              <div class="info">
                <div class="row">
                  <div class="col-lg-3 col-md-6 app">
                    <h2>احصل علي التطبيق</h2>
                    <a href="/" ><img src="{{asset('assets/images/google.png')}}"></a>
                    <ul class="social">
                      <li><a href="/"><i class="fab fa-facebook-f"></i></a></li>
                      <li><a href="/"><i class="fab fa-instagram"></i></a></li>
                      <li><a href="/"><i class="fab fa-twitter"></i></a></li>
                    </ul>
                  </div>
                  <div class="col-lg-3 col-md-6 help">
                    <h2>مساعدة</h2>
                    <ul>
                      <li><a href="{{route('users.getContact')}}" >تواصل معنا</a></li><br>
                      <li><a href="/" >شروط الاستخدام</a></li><br>
                      <li><a href="/" >الخصوصية</a></li><br>
                      <li><a href="./specializization.html" >اعرف التخصصات</a></li><br>
                    </ul>
                  </div>
                  <div class="col-lg-3 col-md-6 lawyer">
                    <h2>انضم لمحاميك</h2>
                    <ul>
                      <li><a href="{{route('lawyers.getLogin')}}">هل انت محامي؟</a></li>
                    </ul>
                    
                  </div>
                  <div class="col-lg-3 col-md-6 logo">
                    <a href="index2.html"><img src="{{asset('assets/lawyerDashboard/src/img/logo.png')}}" alt="محاميك"></a>
                    <ul>
                      <li><a href="/">فريق محاميك</a></li><br>
                      <li><a href="/">معلومات عنا</a></li>
                    </ul>
                  </div>
          
                </div>
              </div>
            </div>
            
          </div>
        </footer>
        <!--start footer-->